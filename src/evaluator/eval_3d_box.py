...
# @numba.jit(nopython=True, parallel=True)
def d3_box_overlap_kernel_iou(boxes,
                          qboxes,
                          iou,
                          criterion=-1,
                          z_axis=1,
...
def d3_box_overlap_kernel_iou_cpu(boxes,
                          qboxes,
                          iou_cpu,
                          criterion=-1,
                          z_axis=1,
...
def d3_box_overlap(boxes, qboxes, criterion=-1, z_axis=1, z_center=1.0):
    ...
    bev_boxes, bev_qboxes = boxes[:, bev_axes], qboxes[:, bev_axes]
    
    iou_cpu_mat = np.zeros((boxes.shape[0], qboxes.shape[0]), dtype=np.float32)
    for i, (box, qbox, bev_box, bev_qbox) in enumerate(zip(boxes,qboxes, bev_boxes, bev_qboxes)):
        iou_cpu = rotate_iou_cpu_eval(bev_box.unsqueeze(0), bev_qbox.unsqueeze(0), 2)
        d3_box_overlap_kernel_iou_cpu(box.unsqueeze(0), qbox.unsqueeze(0), iou_cpu, criterion, z_axis, z_center)
        iou_cpu_mat[i,i] = iou_cpu[0][0]
...
def eval_class(gt_annos,
    ...
    num_examples = len(gt_annos)
    split_parts = get_split_parts(num_examples, num_parts)
    # Compute overlap between each ground truth and prediction
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    if i == 227 : 
                        print("Index at 226")
                    # Assign one prediction to one ground truth with overlap data
...
def do_eval_v3(gt_annos,
    ...
    for i in range(3):
        if i == 2:
            print("Index 2 :  We compute in 3D")
        ret = eval_class(
            gt_annos,
            dt_annos,
            current_classes,
